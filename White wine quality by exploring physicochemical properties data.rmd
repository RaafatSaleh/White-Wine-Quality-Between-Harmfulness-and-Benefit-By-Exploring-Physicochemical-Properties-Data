...
# Load the Data
whit_w <- read.csv("DataSet - wineQualityWhites.csv", sep = ",")
```
...
[Cortez et al., 2009].  Due to privacy and logistic issues, only physicochemical (inputs) and sensory (the output) variables are available (e.g. there is no data
about grape types, wine brand, wine selling price, etc.). Also, we are not sure
...
- Number of Attributes (variables): 11 + output attribute + serial No 
  (x variable)
